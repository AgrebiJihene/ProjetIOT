...
function init() {
    ...
    var which_esps = [
        "80:7D:3A:FD:C5:98",
        "24:0A:C4:96:CB:84", //Nour
        "80:7D:3A:FD:E8:E8" // Mr Menez
...
function process_esp(which_esps,i){
   ...
   /* window.setInterval(get_samples,
    ...
    window.setInterval(get_samples,
        refreshT,
        '/esp/M1Miage2022',     // param 1 for get_samples()
        chart1.series[i], chart2.series[i],// param 2 for get_samples()
        esp);
...
function get_samples(path_on_node, serie1,serie2, who){
    ...
    node_url = 'https://iot22111322m1.herokuapp.com'
	...
	data: {who}, // parameter of the GET request
		...
		listeData1.push([Date.parse(element.date),element.valueTemperature]);
        listeData2.push([Date.parse(element.date),element.valueLight]);

		//listeData.push([Date.now(),element.value]);

            });
            serie1.setData(listeData1); //serie.redraw();
            serie2.setData(listeData2); //serie.redraw();
